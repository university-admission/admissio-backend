...
public class OfferController {
    ...
    @Operation(
            summary = "Отримати інформацію про всі відстежування пропозиції",
            description = "Повертає список пропозицій, зі місцем користувача у кожній з них."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Успішне отримання списку пропозицій",
                    content = @Content(
                            mediaType = "application/json",
                            array = @ArraySchema(
                                    schema = @Schema(implementation = TrackedOfferAnalyticsDto.class)
                            )
                    )
            )
    })
    @PostMapping("/tracked")
    public ResponseEntity<List<TrackedOfferAnalyticsDto>> getAllTrackedOffers(
            @Parameter(description = "Запит, що містить бали користувача, та список відстежуваних пропозицій")
            @RequestBody TrackedOffersRequestDto requestDto) {
